...
OUT_DIR = BASE_DIR / "EXPERIMENTO_FTSE100" / "resultados"
...
def run_experiment():
    ...
    print(f"Procesando {len(files)} activos del FTSE 100 (Batch Mode)...")
    
    for f in files:
        ticker = f.name.split("_")[0]
        if ticker == "^FTSE": continue
        ...
        for i_idx, pred in enumerate(preds):
            ...
            if p_idx == 1 and pred[p_idx] > PROB_UMBRAL:
                fecha = df.index[i]
                try: is_bull = bench.loc[fecha.floor('D'), 'Close'] > bench.loc[fecha.floor('D'), 'sma200']
                except: is_bull = True
                entry_price = c[i]
                if is_bull:
    ...
    if results:
        final_df = pd.concat(results); final_df.to_csv(OUT_DIR / "trades_ftse.csv", index=False)
