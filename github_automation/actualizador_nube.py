...
def build_windows_live(df_1h):
    ...
    o = df_1h.get("Open", c).values.astype(np.float32)
...
def main():
    print("AGENTE TRADING CLOUD - ACTUALIZANDO DATOS...")
    if not CACHE_PATH.exists() or not MODEL_PATH.exists():
        print("ERROR: Faltan archivos en el repositorio (cache o modelo).")
        return
    ...
    for tk in tickers:
        ...
        try:
            ...
            # Reconstrucción segura (NpzFile no tiene .get(), usamos comprobación 'in')
            ...
            if X is not None:
                new_preds = model.predict(X, verbose=0).astype(np.float16)
                new_cache[f"preds_{tk}"] = np.concatenate([old_data[f"preds_{tk}"], new_preds])
