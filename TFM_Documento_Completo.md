...
El sistema exige un rigor estadístico severo para la toma de decisiones: se configuró un Umbral de Confianza (`PROB_UMBRAL = 0.93`). La red debe estar segura al menos en un 93% de que el patrón inminente pertenece a una de las clases alcistas para que el evento se considere matemáticamente digno de inversión.
...
Tras un proceso de optimización iterativo, la investigación concluyó que la máxima eficiencia del capital se alcanza mediante una estrategia de **Regime Switching** (Cambio de Régimen). Este algoritmo introduce un filtro de salud macro-estructural que altera dinámicamente el comportamiento del bot según el estado del índice de referencia (IBEX 35) respecto a su media móvil simple de 200 sesiones (SMA200).

La arquitectura operativa se bifurca en dos modos de gestión:

1. **Régimen Alcista (Bull Market):** Se activa cuando el IBEX 35 cotiza por encima de su SMA200. En este estado, el sistema asume una postura de "paciencia estratégica":
   - **Horizonte Extendido:** Se permite una permanencia máxima de hasta 150 horas.
   - **Salida Dinámica:** Se activa un motor de *Trailing Stop* (5.5%) que permite acompañar la tendencia y capturar el máximo recorrido del patrón detectado por la IA.
...
2. **Régimen Bajista/Lateral (Bear Market):** Se activa cuando el índice cotiza por debajo de su SMA200. El sistema muta hacia una postura de "defensa y rapidez":
   - **Horizonte Comprimido:** Salida obligatoria por tiempo a las 48 horas de mercado.
   - **Salida Fija:** Se emplea un *Stop Loss* estructural (4.7%) sin seguimiento dinámico, minimizando el tiempo de exposición a la volatilidad sistémica y al *drawdown* del mercado.

**Resultados Consolidados de la Estrategia Final:**
- **Capital Final (Neto):** 168,822.00 EUR
- **Beneficio Neto Acumulado:** +68,822.00 EUR
- **Retorno sobre Inversión (ROI):** +68.80%
...
3. **IA Propuesta: Regime Switching (+68.8%):** La variante definitiva que, al integrar el filtro de SMA200 y la gestión dual, logra optimizar el aprovechamiento de las tendencias, batiendo al mercado por un margen masivo.
...
- El **Régimen de Mercado Actual** (Bull/Bear) basado en el filtro de la SMA de 200 días del IBEX 35.
- El **Top 8 de Oportunidades** ordenadas por confianza de la red neuronal.
- Las métricas de **Volumen Efectivo** y el **Tier de Liquidez** asignado a cada activo.
...
Como test de estrés definitivo, se sometió al modelo al mercado de criptoactivos (Top 5 por capitalización: BTC, ETH, SOL, BNB y XRP). Utilizando el script `ga_crypto.py`, se descubrió que el modelo mantiene su ventaja estadística, alcanzando un **Profit Factor de 1.60**. La clave del éxito en este mercado reside en la expansión del Trailing Stop hasta el **13.89%** y una reducción del horizonte de salida a **25 horas**, demostrando que la morfología de las señales es idéntica pero su "velocidad de maduración" es significativamente mayor.
