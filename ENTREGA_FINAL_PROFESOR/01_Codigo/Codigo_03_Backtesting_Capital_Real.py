        ...
        with open(txt_path, 'r', encoding='latin1') as f:
            for line in f:
                ...
                try:
                    ...
                    ticker_data[ticker].append({'date': date, 'high': h, 'low': l, 'close': c, 'volume': v})
                except: continue
...
def main():
    ...
    print(f"  Tiers Alloc:     T1 15%, T2 8%, T3 4% (Rebalanceo Trimestral Lagged)")
    ...
    for tk in sorted(meta["ticker"].tolist()):
        try:
            ...
            if tk in txt_prices:
                ...
                v = df_r['volume'].values.astype(np.float32)
            else:
                c = data[f"c_{tk}"]
                l = data[f"l_{tk}"]
                v = data[f"v_{tk}"]
            cache[tk] = dict(preds=preds, idx=idx, c=c, l=l, v=v)
        except: pass
